...
get_sample_details<-function(labeled_pop,index_list,p_val){
    ...
    for (j in 1:2000){
        x1<-sum(labeled_pop[index_list[[j]],'flag']=="blue")
        p<-(x1+2)/(n+4)
        sample_props[j]<-p
        se<-sqrt(p*(1-p)/n)
        ymins[j]<-max(0,p-1.96*se)
        ymaxes[j]<-min(p+1.96*se,1)
...
start_sample_details<-get_sample_details(start_pop_l, 
                                         start_index_list,
                                         .3)
...
start_sample_plot<-get_sample_plot(start_pop_l, 
                                   start_index_list,
                                   0)
...
get_conf_int<-function(pop_details, sample_num, p_val){
    if (sample_num==0){
        get_ci_start(p_val)
    } else {
